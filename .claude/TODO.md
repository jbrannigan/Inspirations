...
- [x] Collection lifecycle polish: owner hide/restore/delete actions stay centralized in `Manage Visibility`; no per-collection sidebar actions for now.
- [x] Scope hierarchy copy: `Hide in this collection` vs. `Hide globally` vs. `Keep` (corpus-level).
